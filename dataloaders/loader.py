...
def sample_pointcloud(data_path, num_point, pc_attribs, pc_augm, pc_augm_config, scan_name,
    ...
    xyz = data[:, 0:3]
    rgb = data[:, 3:6]
    labels = data[:, 6].astype(np.longlong)
...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index, n_way_classes=None, is_test=False):
        ...
        support_ptclouds, support_masks, query_ptclouds, query_labels = self.generate_one_episode(sampled_classes, is_test=is_test)
    ...
    def generate_one_episode(self, sampled_classes, is_test=False):
        support_ptclouds = []
        support_masks = []
        query_ptclouds = []
        query_labels = []
...
def batch_train_task_collate(batch):
    ...
    task_valid_support_ptclouds, task_valid_support_masks, task_valid_query_ptclouds, task_valid_query_labels = list(
        zip(*batch))
        ...
        if os.path.exists(test_data_path):
            ...
            for sampled_classes in class_comb:
                ...
                for i in range(num_episode_per_comb):
                    data = dataset.__getitem__(episode_ind, sampled_classes, is_test=True)
...
def batch_test_task_collate(batch):
    batch_support_ptclouds, batch_support_masks, batch_query_ptclouds, batch_query_labels, batch_sampled_classes = \
    batch[0]

    data = [torch.from_numpy(batch_support_ptclouds).transpose(2, 3), torch.from_numpy(batch_support_masks.astype(np.longlong)),
            torch.from_numpy(batch_query_ptclouds).transpose(1, 2),
            torch.from_numpy(batch_query_labels.astype(np.longlong))]
