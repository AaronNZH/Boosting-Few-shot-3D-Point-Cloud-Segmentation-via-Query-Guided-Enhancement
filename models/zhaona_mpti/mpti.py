...
from models.attention import SelfAttention, MultiHeadAttention
...
class MultiPrototypeTransductiveInference(nn.Module):
    def __init__(self, args):
        ...
        self.proto_attn = MultiHeadAttention(in_channel=self.feat_dim, out_channel=self.feat_dim, n_heads=2,
                                             att_dropout=args.dropout_ratio, use_proj=False, use_ffn=False, is_cross=True)
        self.embedding_fusion = nn.Conv1d(args.edgeconv_widths[0][-1] + args.output_dim + args.base_widths[-1], self.feat_dim, 1)
    ...
    def getFeatures(self, x):
        ...
        if self.use_attention:
            ...
            out_feat = torch.cat((feat_level1, att_feat, feat_level3), dim=1)
            return self.embedding_fusion(out_feat)
            # return out_feat
        else:
            ...
            out_feat = torch.cat((feat_level1, map_feat, feat_level3), dim=1)
            return self.embedding_fusion(out_feat)
            # return out_feat
